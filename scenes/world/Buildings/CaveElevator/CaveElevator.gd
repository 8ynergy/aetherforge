...
@onready var closed_texture: Texture2D = preload("res://scenes/world/Buildings/CaveElevator/CaveElevatorClosed.png")
@onready var opened_texture: Texture2D = preload("res://scenes/world/Buildings/CaveElevator/CaveElevatorOpen.png")
...
func _on_input_event(_viewport: Node, event: InputEvent, _shape_idx: int) -> void:
	if event is InputEventMouseButton and event.pressed and event.button_index == MOUSE_BUTTON_LEFT:
		...
		if current_scene == "Camp.tscn":
			var target_scene_path = "res://scenes/main/Mine.tscn"
			get_tree().change_scene_to_file(target_scene_path)
			if mat:
				mat.set_shader_parameter("OnHoverShader", false)
